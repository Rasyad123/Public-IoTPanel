...
<head>
  ...
  <style>
    ...
    .modal-content {
      ...
      max-height: 90vh;
      /* Tambahkan ini untuk batasi tinggi konten */
    }
...
<body>
  ...
  <div class="container">
    ...
    <div class="device-control" id="hilangwak">
      <h2>Controls</h2>
      <div class="control-grid ">
        <div class="control-box">
          <span class="control-label">Lamp</span>
          <label class="switch">
            <input type="checkbox" id="ESP32_01_TogLED_03" onclick="GetTogBtnLEDState('ESP32_01_TogLED_03')">
    ...
    <div class="device-control" id="otowak" style="display: none;">
      <h2>Controls</h2>
      <h2 style="font-size: 28px; color: #27ae60; margin: 0;">Auto Mode Enabled</h2>
    </div>

    <div class="footer">
      <span>Last updated: <span id="ESP32_01_LTRD"></span></span>
      <div class="buttons">
        <button class="circle-button" id="informasi" title="infomint"><i class="bi bi-info-circle"></i></button>
        <button class="circle-button" id="infowak" title="Btn Otomatis"><i class="bi bi-repeat"></i></button>
        <button class="circle-button" title="Calendar" onclick="window.open('recordtable.php', '_blank');">📅</button>
  ...
  <div id="infoModal" class="modal">
    <div class="modal-content">
      ...
      <h2>Smart Farming Dashboard Info</h2>
      <p>Selamat datang di Smart Farming Dashboard!</p>
      <ul>
        <li>🌡️ Temperature: Mengukur suhu lingkungan</li>
        <li>💧 Humidity: Mengukur kelembaban udara</li>
        <li>🌱 Soil Moisture: Mengukur kelembaban tanah</li>
        <li>🔆 LDR (Light): Mengukur intensitas cahaya</li>
      </ul>
      <p>Kontrol perangkat: Anda dapat mengaktifkan/menonaktifkan Lampu, Kipas, dan Pompa Air.</p>
      <p>Mode Otomatis tersedia by default</p>
    </div>
  </div>


  <script>
    // Ambil elemen tombol, modal, dan tombol tutup
    const infoButton = document.getElementById('informasi'); // Pastikan ada tombol dengan id "informasi"
  ...
  <script>
    const infoToggle = document.getElementById('infowak');
    const infoBox1 = document.getElementById('hilangwak');
    const infoBox2 = document.getElementById('otowak');

    // Set initial icon state (opsional: bisa simpan ke localStorage jika perlu)
    let infoState = false;
    ...
    infoToggle.addEventListener('click', () => {
      ...
      if (infoState) {
        ...
        infoToggle.innerHTML = '<i class="bi bi-repeat-1"></i>';
      } else {
        infoBox1.style.display = 'block';
        infoBox2.style.display = 'none';
        infoToggle.innerHTML = '<i class="bi bi-repeat"></i>';
      }
    });
  </script>
  ...
  <script>
      ...
      xmlhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
          ...
          if (myObj.id == "esp32_01") {
            ...
            if (myObj.LED_03 == "ON") {
              document.getElementById("ESP32_01_TogLED_03").checked = true;
            } else if (myObj.LED_03 == "OFF") {
              document.getElementById("ESP32_01_TogLED_03").checked = false;
            }
          }
        }
      };
    ...
    function GetTogBtnLEDState(togbtnid) {
      ...
      if (togbtnid == "ESP32_01_TogLED_03") {
        var togbtnchecked = document.getElementById(togbtnid).checked;
        var togbtncheckedsend = "";
        if (togbtnchecked == true) togbtncheckedsend = "ON";
        if (togbtnchecked == false) togbtncheckedsend = "OFF";
        Update_LEDs("esp32_01", "LED_03", togbtncheckedsend);
      }
    }
    //------------------------------------------------------------
  ...
  <script>
    document.addEventListener("DOMContentLoaded", function() {
      ...
      if (infoButton && popupModal && closeModal) {
